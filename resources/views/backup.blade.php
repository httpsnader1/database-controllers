...
<div class="space-y-8 animate-fadeIn" x-data="{
    ...
    <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative group">
        ...
        <div class="hidden lg:block w-64 h-64 bg-indigo-50/50 rounded-full absolute -end-20 -bottom-20 pointer-events-none transition-transform group-hover:scale-110"></div>
        ...
        <div x-show="showImportModal" x-cloak class="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden" @click.away="!isImporting && (showImportModal = false)">
                 <div class="p-8 text-center" x-show="!isImporting">
                    ...
                    <p class="text-slate-500 text-sm leading-relaxed mb-6" x-html="'{{ __('database-controllers::messages.select_sql_file') }}'.replace('.sql', '<span class=\'font-bold text-slate-800\'>.sql</span>')"></p>
                    ...
                    <form action="{{ route('database-controllers.backup.import') }}" method="POST" enctype="multipart/form-data" @submit="isImporting = true">
                        ...
                        <div class="flex flex-col space-y-3">
                            <button type="submit" class="w-full py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-600/20 active:scale-95 transition">{{ __('database-controllers::messages.restore_now_btn') }}</button>
                            <button type="button" @click="showImportModal = false" class="w-full py-3 text-slate-400 font-bold hover:bg-slate-50 rounded-xl transition">{{ __('database-controllers::messages.cancel') }}</button>
                        </div>
                    </form>
                </div>
                ...
                <div class="p-8 text-center" x-show="!isRestoring">
                    ...
                    <div class="flex flex-col space-y-2">
                        <form :action="'{{ route('database-controllers.backup.restore', 'FILENAME') }}'.replace('FILENAME', restoringBackup)" method="POST" class="w-full" @submit="isRestoring = true">
                            @csrf
                            <button type="submit" class="w-full py-4 bg-emerald-600 text-white rounded-2xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-600/20 active:scale-95">{{ __('database-controllers::messages.restore_confirm_btn') }}</button>
                        </form>
                        <button @click="showRestoreModal = false" class="w-full py-3 text-slate-400 font-bold hover:bg-slate-50 rounded-xl transition">{{ __('database-controllers::messages.changed_my_mind_btn') }}</button>
                    </div>
                </div>
                ...
                <div class="p-8 text-center">
                    ...
                    <div class="flex flex-col space-y-2">
                        <form :action="'{{ route('database-controllers.backup.delete', 'FILENAME') }}'.replace('FILENAME', deletingBackup)" method="POST" class="w-full">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full py-4 bg-red-600 text-white rounded-2xl font-bold hover:bg-red-700 transition shadow-lg shadow-red-600/20 active:scale-95">{{ __('database-controllers::messages.delete_backup_permanently_btn') }}</button>
                        </form>
                        <button @click="showDeleteModal = false" class="w-full py-4 text-slate-400 font-bold hover:bg-slate-50 rounded-2xl transition">{{ __('database-controllers::messages.cancel') }}</button>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="p-8">
                    ...
                    <div class="bg-indigo-50/50 border border-indigo-100 p-4 rounded-xl mb-8 flex items-start">
                        ...
                        <div class="ltr:ml-4 rtl:mr-4">
                            ...
                            <p class="text-[11px] text-indigo-600 leading-relaxed" x-html="'{{ __('database-controllers::messages.exclude_tables_desc') }}'.replace('mysqldump', '<strong>mysqldump</strong>')">
                            </p>
                        </div>
                    </div>
