...
<div class="animate-fadeIn grid grid-cols-1 gap-8 items-start">
    ...
    <!-- Left: Stats List (Each in a line) -->
     <div class="grid grid-cols-1 gap-4">
        <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest ml-2">Database Summary</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            @foreach($stats as $label => $value)
                <div class="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex items-center group hover:border-indigo-500/50 hover:shadow-md transition-all duration-300">
                    <div class="w-10 h-10 rounded-xl @if($loop->index == 0) bg-indigo-50 text-indigo-600 @elseif($loop->index == 1) bg-emerald-50 text-emerald-600 @elseif($loop->index == 2) bg-amber-50 text-amber-600 @else bg-rose-50 text-rose-600 @endif flex items-center justify-center mr-4 group-hover:scale-110 transition-transform">
            ...
            <div class="space-y-3">
                @foreach($dbInfo as $key => $value)
                    <div class="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-0 hover:bg-slate-50 px-3 rounded-lg transition group">
                        ...
                        <span class="text-xs font-mono font-bold @if($key == 'Database') text-indigo-600 bg-indigo-50 px-2 py-1 rounded @else text-slate-700 @endif group-hover:scale-105 transition-transform origin-right">
                            {{ $value }}
                        </span>
                    </div>
                @endforeach
